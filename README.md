...
- **Gate spotlight**: `light_entity` bolts a bracket spot to the post (recessed in the lintel for `door`), throws a beam onto the leaf when it is on, and adds a *Light* button that stays available even while the gate moves.
...
| `show_spot` | Draw the spotlight and offer its button. Default `true`; `false` hides both. |
